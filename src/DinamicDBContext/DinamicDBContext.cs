...
namespace HudsonVentura { 
    public class DinamicDBContext : IDisposable
    {
        public DbConnection _conn { get; set; }
        public bool isConnected { get; private set; }


        public DinamicDBContext(DbConnection connection)
        {
            _conn = connection;
            ...
            {  
                foreach (PropertyInfo pro in temp.GetProperties())  
                {
                    if (pro.Name.ToLower() == column.ColumnName.ToLower()) {
                        ...
                        var tipo = dr[column.ColumnName].GetType().Name;
                        try
                        {
                            switch (tipo)
                            {
                        ...
                        {
                            throw new Exception($"Converta a propriedade {column.ColumnName} da sua classe de domínio {obj.GetType().Name} para o tipo {tipo}", error);
                        }
